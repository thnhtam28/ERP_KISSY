...
namespace Erp.Domain.Account.Repositories
{
    public class CustomerRepository : GenericRepository<ErpAccountDbContext, Customer>, ICustomerRepository
    {
        ...
        public List<vwCustomer> GetListAllvwCustomer()
        {
            return Context.vwCustomer.Where(item => (item.IsDeleted == null || item.IsDeleted == false)).ToList();
        }
        /// <summary>
        ...
        public Customer GetCustomerByCode(string Phone)
        {
            return Context.Customer.SingleOrDefault(item => item.Code == Phone && (item.IsDeleted == null || item.IsDeleted == false));
        }
