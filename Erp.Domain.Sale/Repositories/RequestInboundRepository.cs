...
namespace Erp.Domain.Sale.Repositories
{
    public class RequestInboundRepository : GenericRepository<ErpSaleDbContext, RequestInbound>, IRequestInboundRepository
    {
        ...
        public RequestInbound GetvwRequestInboundByIdOutbound(int Id)
        {
            return Context.RequestInbound.SingleOrDefault(item => item.OutboundId == Id && (item.IsDeleted == null || item.IsDeleted == false));
        }

        public RequestInbound GetvwRequestInboundByIdInbound(int Id)
        {
            return Context.RequestInbound.SingleOrDefault(item => item.InboundId == Id && (item.IsDeleted == null || item.IsDeleted == false));
        }
