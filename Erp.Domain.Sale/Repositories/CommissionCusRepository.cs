...
namespace Erp.Domain.Sale.Repositories
{
    public class CommissionCusRepository : GenericRepository<ErpSaleDbContext, CommissionCus>, ICommissionCusRepository
    {
        ...
        public IQueryable<vwCommissionCus> GetAllvwCommissionCus()
        {
            return Context.vwCommissionCus.Where(item => (item.IsDeleted == null || item.IsDeleted == false));
        }

        public IQueryable<vwCommissionCus_ch> GetAllvwCommissionCus_ch()
        {
            return Context.vwCommissionCus_ch.Where(item => (item.IsDeleted == null || item.IsDeleted == false));
        }
        public List<vwCommissionCus_ch> GetListAllvwCommissionCus_ch()
        {
            return Context.vwCommissionCus_ch.Where(item => (item.IsDeleted == null || item.IsDeleted == false)).ToList();
        }
        /// <summary>
