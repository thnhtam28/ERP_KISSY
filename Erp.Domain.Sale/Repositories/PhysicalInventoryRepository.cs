...
namespace Erp.Domain.Sale.Repositories
{
    public class PhysicalInventoryRepository : GenericRepository<ErpSaleDbContext, PhysicalInventory>, IPhysicalInventoryRepository
    {
        ...
        public IQueryable<PhysicalInventoryDetailchildren> GetAllPhysicalInventoryDetailchildren(int PhysicalInventoryId)
        {
            return Context.PhysicalInventoryDetailchildren.Where(item => item.PhysicalInventoryDetailId == PhysicalInventoryId && (item.IsDeleted == null || item.IsDeleted == false));
        }

      

        public PhysicalInventoryDetailchildren GetPhysicalInventoryDetailchildrenById(int Id)
        {
            return Context.PhysicalInventoryDetailchildren.SingleOrDefault(item => item.Id == Id && (item.IsDeleted == null || item.IsDeleted == false));
        }

        public void InsertPhysicalInventoryDetailchildren(PhysicalInventoryDetailchildren PhysicalInventoryDetailchildren)
        {
            Context.PhysicalInventoryDetailchildren.Add(PhysicalInventoryDetailchildren);
            Context.Entry(PhysicalInventoryDetailchildren).State = EntityState.Added;
            Context.SaveChanges();
        }

        public void DeletePhysicalInventoryDetailchildren(int Id)
        {
            PhysicalInventoryDetailchildren deletedPhysicalInventoryDetailchildren = GetPhysicalInventoryDetailchildrenById(Id);
            Context.PhysicalInventoryDetailchildren.Remove(deletedPhysicalInventoryDetailchildren);
            Context.Entry(deletedPhysicalInventoryDetailchildren).State = EntityState.Deleted;
            Context.SaveChanges();
        }

        public void DeletePhysicalInventoryDetailchildrenRs(int Id)
        {
            PhysicalInventoryDetailchildren deletePhysicalInventoryDetailchildrenRs = GetPhysicalInventoryDetailchildrenById(Id);
            deletePhysicalInventoryDetailchildrenRs.IsDeleted = true;
            UpdatePhysicalInventoryDetailchildren(deletePhysicalInventoryDetailchildrenRs);
        }

        public void UpdatePhysicalInventoryDetailchildren(PhysicalInventoryDetailchildren PhysicalInventoryDetail)
        {
