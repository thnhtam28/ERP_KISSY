...
namespace Erp.Domain.Sale.Repositories
{
    public class ProductInvoiceRepository : GenericRepository<ErpSaleDbContext, ProductInvoice>, IProductInvoiceRepository
    {
        ...
        {
            List<vwProductInvoice> kq = new List<vwProductInvoice>();
            
            return kq = Context.vwProductInvoice.Where(item => (item.IsDeleted == null || item.IsDeleted == false)).ToList();
        }
        /// <summary>
        ...
        {
            return Context.vwProductInvoice.Take(1).SingleOrDefault(item => item.Code == code && (item.IsDeleted == null || item.IsDeleted == false));
        }
        ...
        public vwProductInvoiceDetail GetvwProductInvoiceDetailById(int Id)
        {
