...
namespace Erp.BackOffice.Sale.Controllers
{
    ...
    public class ProductInboundController : Controller
    {
        ...
        public ViewResult Index(string txtCode, string txtMinAmount, string txtMaxAmount, string txtPurchaseOrder, int? warehouseDestinationId, string txtWarehouseSource, string startDate, string endDate, string txtProductCode, int? supplierId, string Status)
        {
            ...
            IEnumerable<ProductInboundViewModel> q = ProductInboundRepository.GetAllvwProductInboundFull().Where(x => x.BranchId == intBrandID || intBrandID == 0 || x.BranchId_khonhan == intBrandID).AsEnumerable()
            ...
            if (string.IsNullOrEmpty(txtPurchaseOrder) == false || warehouseDestinationId != null || string.IsNullOrEmpty(txtWarehouseSource) == false)
            {
        ...
        public ActionResult Create(int? PurchaseOrderId)
        {
            ...
            if (PurchaseOrderId != null && PurchaseOrderId.Value > 0)
            {
                ...
                model.CodeOrderRed = aaa.CodeOrderRed;
                model.DateOrder = aaa.DateOrder;
            ...
            var orderList = PurchaseOrderRepository.GetAllPurchaseOrder().AsEnumerable()
                .Where(x => x.SupplierId != null
                   && (x.Status == Wording.OrderStatus_pending || x.Status == Wording.OrderStatus_shipping || x.BranchId == intBrandID || intBrandID == 0))
            ...
            if (ModelState.IsValid)
            {
                ...
                using (var scope = new TransactionScope(TransactionScopeOption.Required))
                {
                    try
                    {
                        ...
                        {
                            TempData["Message"] = "Chưa chọn sản phảm";
                            return RedirectToAction("Create", "ProductInbound");
                        }
        ...
        public ActionResult Edit(int? Id, string TransactionCode)
        {
            ...
            if (ProductInbound.IsDeleted == true)
            {
        ...
        public void Archive(ProductInbound productInbound)
        {
            ...
            if (string.IsNullOrEmpty(check))
            {
                ...
                productInbound.ModifiedUserId = Helpers.Common.CurrentUser.Id;
                ProductInboundRepository.UpdateProductInbound(productInbound);
            }
            //else
            //{
        ...
        public ActionResult PrintBarcode(int? Id)
        {
            ...
            if (ProductInbound != null)
            {
                ...
                template = templatePrintRepository.GetAllTemplatePrint().Where(x => x.Code.Contains("BarcodeProductInBound")).OrderByDescending(x => x.CreatedDate).FirstOrDefault();
                if (template == null)
                    return Content("No template!");
                ...
                {
                    var BranchName = branchRepository.GetBranchById(ProductInbound.BranchId.Value);
                    output = output.Replace("{BranchName}", BranchName.Code);
                }
                output = output.Replace("{WarehouseDestinationName}", ProductInbound.WarehouseDestinationName);
                output = output.Replace("{ProductInboundDate}", ProductInbound.CreatedDate.Value.ToString("dd/MM/yyyy HH:mm"));
                //Tạo barcode
        ...
        {
            var ProductInbound = ProductInboundRepository.GetvwProductInboundById(Id);
        ...
        public JsonResult ImportExcel(List<sanphamexcel> listsp)
        {
            ...
            "<span style = \"display:block; font-size:11px;\" class=\"field-validation-valid help-inline\" data-valmsg-for=\"DetailList_" + item.Quantity + "_Quantity\" data-valmsg-replace=\"false\"></span></td>";
                    ...
                    a += "<td hidden class=\"has-error detail-product-price \">" +
        ...
        "<span style = \"display:block; font-size:11px;\" class=\"field-validation-valid help-inline\" data-valmsg-for=\"DetailList_" + itemxoa.Quantity + "_Quantity\" data-valmsg-replace=\"false\"></span></td>";
                ...
                a1 += "<td hidden class=\"has-error detail-product-price \">" +
