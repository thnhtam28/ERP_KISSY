...
namespace Erp.BackOffice.Sale.Controllers
{
    ...
    public class PhysicalInventoryController : Controller
    {
        ...
        public ActionResult Index(string txtSearch, string txtWarehouseName)
        {
            ...
            IEnumerable<PhysicalInventoryViewModel> list = PhysicalInventoryRepository.GetAllvwPhysicalInventory().Where(x => x.BranchId == intBrandID || intBrandID == 0)
            ...
            if (string.IsNullOrEmpty(txtSearch) == false || string.IsNullOrEmpty(txtWarehouseName) == false)
            {
                ...
                list = list.Where(x => x.Code.ToLower().Contains(txtWarehouseName) || Erp.BackOffice.Helpers.Common.ChuyenThanhKhongDau(x.WarehouseName).Contains(txtWarehouseName));
            }
            ViewBag.Tongdong = list.Count();
        ...
        public ActionResult Create()
        {
            ...
            var warehouseList = WarehouseRepository.GetAllWarehouse().Where(x => x.BranchId == intBrandID || intBrandID == 0).AsEnumerable()
                        ...
                        if (model.DetailList != null)
                        {
                            ...
                            foreach (var item in model.DetailList)
                            {
                                ...
                                if (inventoryProduct.Count() > 0)
                                {
                                    ...
                                    // khi tao kiểm kê máy thì ko updtate sl kho
        ...
        //[HttpPost]
        //public ActionResult Kiemtrabangmay(List<CustomerTest> listsp)
        //{
        ...
        //        try
        //        {
        //            //buoc 1 lay ton kho hien tai len
        //            var PhysicalInventory_detail = PhysicalInventoryRepository.GetAllvwPhysicalInventoryDetail(int.Parse(listsp[1].Giacu.ToString())).ToList();

        //            if (PhysicalInventory_detail != null)
        //            {
        //                foreach (var item in PhysicalInventory_detail)
        //                {

        //                    // tim duoc vi tri cua detail
        //                    foreach (var item1 in listsp)
        //                    {
        //                        if (item1.Id == item.Id)
        //                        {
        //                            PhysicalInventoryDetail physicalInventoryDetail = PhysicalInventoryRepository.GetPhysicalInventoryDetailById(item.Id);
        //                            physicalInventoryDetail.QuantityRemaining = physicalInventoryDetail.QuantityRemaining + int.Parse(item1.Giamoi.ToString());
        //                            physicalInventoryDetail.QuantityDiff = physicalInventoryDetail.QuantityRemaining - physicalInventoryDetail.QuantityInInventory;
        //                            PhysicalInventoryRepository.UpdatePhysicalInventoryDetail(physicalInventoryDetail);
        //                        }
                        ...
                        foreach (var item in PhysicalInventory_detail)
                        {
                            ...
                            foreach (var item1 in listsp)
                            {
                                if (item1.Id == item.Id)
                                {
                                   ...
                                   // physicalInventoryDetailchildren.ProductId = 
                                    //physicalInventoryDetailchildren.ProductCode = 
                                    //
                                    ...
                                    //insert KKC
                                    var listchild = PhysicalInventoryRepository.GetAllPhysicalInventoryDetailchildren(physicalInventoryDetail.PhysicalInventoryId).ToList();
                                    PhysicalInventoryDetailchildren physicalInventoryDetailchildren = new PhysicalInventoryDetailchildren();
                                    physicalInventoryDetailchildren.CreatedDate = DateTime.Now;
                                    physicalInventoryDetailchildren.CreatedUserId =Int32.Parse( item1.iduser);
                                    physicalInventoryDetailchildren.PhysicalInventoryDetailId = physicalInventoryDetail.PhysicalInventoryId;
                                    physicalInventoryDetailchildren.ProductId = physicalInventoryDetail.ProductId;
                                    physicalInventoryDetailchildren.Qty = int.Parse(item1.Giamoi.ToString());
                                    physicalInventoryDetailchildren.NumberCheck = listchild.Count()/ listsp.Count() + 1;
                                    PhysicalInventoryRepository.InsertPhysicalInventoryDetailchildren(physicalInventoryDetailchildren);
                                    //
                                }
        ...
        public ActionResult Kiemtrabangmay(int? Id)
        {
            var PhysicalInventory = PhysicalInventoryRepository.GetAllvwPhysicalInventory()
                .Where(item => item.Id == Id).FirstOrDefault();
            if (PhysicalInventory != null)
            {
                ...
                .ThenBy(item => item.ProductCode)
                .ToList();
                var detailchild = PhysicalInventoryRepository.GetAllPhysicalInventoryDetailchildren(Id.Value).Select(

                    item => new PhysicalInventoryDetailchildrenViewModel
                    {
                        Id = item.Id,
                        CreatedDate = item.CreatedDate,
                        ProductId = item.ProductId,
                ...
                //foreach (var i in detailchild )
                //{
                //    var product = ProductRepository.GetProductById(i.ProductId.Value);
                //    i.ProductCode = product.Code;
                //    i.ProductName = product.Name;
                //}
                var test = detailchild.Count();
                var test3 = new List<PhysicalInventoryDetailchildrenViewModel>();
                if (test > 0)
                {
                    var test2 = detailchild[test - 1].NumberCheck;
                    
                    for (int i = 1; i <= test2; i++)
                    {
                        
                        var aa = detailchild.FirstOrDefault(x => x.NumberCheck == i);
                        var user = userRepository.GetUserById(aa.CreatedUserId);
                        aa.UserName = user.FullName;
                        test3.Add(aa);
                    }
                }
                ViewBag.detailchild = test3;
                return View(model);
            }
        ...
        public ActionResult SuaKiemtrabangmay(int? Id)
        {
            var PhysicalInventory = PhysicalInventoryRepository.GetAllvwPhysicalInventory()
                .Where(item => item.Id == Id).FirstOrDefault();
            if (PhysicalInventory != null)
        ...
        [HttpPost]
        public ActionResult SuaKiemtrabangmay(List<CustomerTest> listsp)
        {
        ...
        [HttpPost]
        public JsonResult GetListJsonDetailById(int? Id, int? Id2)
        {
            if (Id == null)
                return Json(new List<int>(), JsonRequestBehavior.AllowGet);

            //var list = PhysicalInventoryRepository.GetAllPhysicalInventoryDetailchildren(Id2.Value);
            //list = list.Where(x => x.NumberCheck == Id);
            var detailchild = PhysicalInventoryRepository.GetAllPhysicalInventoryDetailchildren(Id2.Value).Select(
            ...
            detailchild = detailchild.Where(x => x.NumberCheck == Id && x.Qty> 0).ToList();
            foreach (var i in detailchild)
            {
                //var user = userRepository.GetUserById(i.CreatedUserId);
                var product = ProductRepository.GetProductById(i.ProductId.Value);

                i.ProductCode = product.Code;
                i.ProductName = product.Name;
                //i.UserName = user.FullName;
            }
            return Json(detailchild, JsonRequestBehavior.AllowGet);
        }
        ...
        public ActionResult Print(int? Id)
        {
            ...
            if (PhysicalInventory != null && PhysicalInventory.IsDeleted != true)
            {
                ...
                {
                    var Row = "<tr style=\"background:#eee\"><td colspan=\"7\"><b>" + item.CategoryCode + "</b></td></tr>";
