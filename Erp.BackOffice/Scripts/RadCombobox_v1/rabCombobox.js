...
(function ($) {
    $.fn.radComboBox = function (options) {
        ...
        // customId: allow to pass a customId incase multiple combo box in the screen
        //settings.customIdtab2
        for (var i = 0; i < elements.length; i++) {
            initComboBox($(elements[i]), settings, settings.customId3 ||settings.customId2||settings.customId||i + 1);
        }
    };
    ...
    function initComboBox($this, settings, rcb_id) {
        ...
        $rcbSlide = initTable(settings, $this, rcb_id);
    ...
    function initTable(settings, $elemSelect, rcb_id) {
        ...
        if (settings.colTitle != '') {
            ...
            for (var index = 1; index <= arr_colTitle.length; index++) {
                ...
                if (settings.colSearch.indexOf(index.toString()) != -1) {
                    search_panel += '<input id="idboxsearch' + rcb_id+'" data-index="' + index + '" class="rcb-input-search ' + class_col_main_search + '" style="width: ' + settings.width + 'px" type="text" placeholder="Tìm kiếm..." autocomplete="off"/>';
                }
            }
            //mcuong.fit
            ul_header += '</tr></table>';
            search_panel += '</div>';
            //alert(settings.rcbname);
            //hoapd them vao de an cot tieu de

            if (typeof settings.rcbname !== "undefined") {
        ...
        var $rcbScroll = $(' <div class="rcbScroll rcbWidth rcb2POS" style="height: ' + settings.listHeight + 'px;"></div>');
    ...
    function initOrUpdateListOption(settings, $elemSelect, $list_option) {
        ...
        for (var iOpt = 0; iOpt < $list_option.length; iOpt++) {
            ...
            var $li_rcbItem = $('<tr class="rcbItem rcbTemplate rcbPOS' + ($(elemOpt).attr('value') == $elemSelect.val() ? 'selected ' : '') + ($(elemOpt).is(':disabled') == true ? 'disabled ' : '') + ($(elemOpt).css('display') == 'none' ? 'hideItem ' : '') + '"></tr>');
            var row_item = '';
    ...
    function initEvent(settings, $elemSelect, $rcbSlide, rcb_id) {
        ...
        $rcbSlide.find('.rcb-input-search').keyup(function () {
            //Kiểm tra txtSearch khách hàng có rỗng không
            if (rcb_id === 2) {
